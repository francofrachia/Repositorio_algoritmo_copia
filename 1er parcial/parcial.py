...
def listar_inverso (lista):
    if not lista:
        ...
        print(lista[-1],end=" ")
        listar_inverso(lista[:-1])
        
lista_a_invertir = ["r", "e", "t", "l", "a", "W"]
print((lista_a_invertir),end=" ")
print("")
listar_inverso(lista_a_invertir)
print("")
...
# a)
def nombre_especie(jedis):
    keys = [(jedi['name'], jedi['species']) for jedi in jedis]
    keys.sort()
    jedis_ordenado = []

    for key in keys:
        jedi = next(jedi for jedi in jedis if (jedi['name'], jedi['species']) == key)
        jedis_ordenado.append(jedi)
    return jedis_ordenado

jedis_ordenado = nombre_especie(jedis)

for jedi in jedis_ordenado:
    print(f"{jedi['name']} - {jedi['species']}")
print("")
# b)
def jedi_info(jedis, nombre):
    for jedi in jedis:
        if jedi['name'] == nombre:
            print(jedi)
            return
    print(f"No se encontró el Jedi {nombre}")

jedi_info(jedis, "Ahsoka Tano")
jedi_info(jedis, "Kit Fisto")
print("")
#c)
def obtener_padawans(jedis, maestro):
    padawans = []
    for jedi in jedis:
        if jedi['master'] == maestro:
            padawans.append(jedi['name'])
    return padawans

yoda_padawans = obtener_padawans(jedis, "Yoda")
luke_padawans = obtener_padawans(jedis, "Luke Skywalker")

print("Padawans de Yoda:", yoda_padawans)
print("Padawans de Luke Skywalker:", luke_padawans)
print("")
# d)
def especies_jedis(jedis, especie):
    jedis_especie = []
    for jedi in jedis:
        if jedi['species'] == especie:
            jedis_especie.append(jedi['name'])
    return jedis_especie

humanos = especies_jedis(jedis, "Human")
twilek = especies_jedis(jedis, "Twi'lek")

print("Jedis humanos:", humanos)
print("Jedis twi'lek:", twilek)
print("")
# e)
def empiezan_con_a(jedis, letra):
    inicial = []
    for jedi in jedis:
        if jedi['name'].startswith(letra):
            inicial.append(jedi['name'])
    return inicial

a_jedis = empiezan_con_a(jedis, "A")

print("Jedis que comienzan con A:", a_jedis)
print("")
# f)
def jedis_varios_colores(jedis):
    varios_colores = []
    for jedi in jedis:
        if jedi['lightsaber_color'] is not None and '/' in jedi['lightsaber_color']:
            varios_colores.append(jedi['name'])
    return varios_colores

varios_colores = jedis_varios_colores(jedis)

print("Jedis con sable de luz de más de un color:", varios_colores)
print("")
# g)
def color_buscado(jedis, color):
    jedis_color = []
    for jedi in jedis:
        if jedi['lightsaber_color'] is not None and color in jedi['lightsaber_color']:
            jedis_color.append(jedi['name'])
    return jedis_color

yellow_jedis = color_buscado(jedis, "Yellow")
purple_jedis = color_buscado(jedis, "Purple")

print("Jedis con sable de luz amarillo:", yellow_jedis)
print("Jedis con sable de luz violeta:", purple_jedis)
print("")
# h)
def encontrar_padawans(jedis, maestro):
    padawans = []
    for jedi in jedis:
...
qui_gon_padawans = encontrar_padawans(jedis, "Qui-Gon Jinn")
mace_windu_padawans = encontrar_padawans(jedis, "Mace Windu")

print("Padawans de Qui-Gon Jin:", qui_gon_padawans)
print("Padawans de Mace Windu:", mace_windu_padawans)
print("")
# i)
def jedis_por_ranking(jedis, ranking):
    jedis_por_rank = []
    for jedi in jedis:
        if jedi['rank'] == ranking:
            jedis_por_rank.append(jedi['name'])
    return jedis_por_rank

grand_masters = jedis_por_ranking(jedis, "Grand Master")

print("Jedis con el rango de Grand Master:", grand_masters)
